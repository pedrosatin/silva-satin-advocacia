...
**Implementado:** o `<iframe>` do Google Maps foi removido do carregamento inicial. Em seu lugar, `#local` mostra um card estático (`.map-placeholder`) com o endereço e um link "Ver no Google Maps ↗" (fallback, abre o endereço em nova aba). O mapa em si carrega **automaticamente, sem exigir clique**: um `IntersectionObserver` injeta o `<iframe>` assim que a seção "Onde estamos" se aproxima da viewport (`rootMargin: 300px`) — mas só dispara depois que `window.load` já ocorreu, garantindo que o mapa nunca dispute banda/CPU com o carregamento inicial mesmo que o usuário role rápido até a seção. Nada do Maps é buscado antes do resto da página terminar de carregar (fallback: em navegadores sem `IntersectionObserver`, carrega assim que `window.load` dispara).
